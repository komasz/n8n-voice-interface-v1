...
async function processRecording(audioBlob, fileName) {
    ...
    try {
        ...
        // Jeśli otrzymaliśmy odpowiedź i URL audio, odtwórz ją
        if (data.response) {
            // Dodaj odpowiedź do transkrypcji
            showResponse(data.response);
            
            // Jeśli mamy bezpośredni URL audio z serwera, użyj go
            if (data.audio_url) {
                // Odtwórz odpowiedź
                await playAudio(data.audio_url);
            } else {
                // W przeciwnym razie generuj TTS dla odpowiedzi
